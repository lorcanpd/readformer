...
#     def __init__(self, emb_dim, n_order, kernel_size, num_heads=4, nonlinearity=None, filter_length=513):
...
#             read_embeddings, read_positions, start_indices, batch_indices = split_into_reads(embeddings, positions)
#         else:
...
#             h_i = filters[i]
#             # test = h_i.unsqueeze(2)
#             # test = test.repeat(1, 1, h_i.shape[1], 1)
#             # Outer product with summation operation
#             # v = F.rearrange(v, 'b h d l -> b h 1 d l')  # Expand dimensions for outer product
#             # v = v.unsqueeze(3)
#             # x_i = rearrange(x_i, 'b h d l -> b h d 1 l')  # Prepare x_i for outer product
#             # x_i = x_i.unsqueeze(2)
#
#             outer_prod = torch.einsum(
#                 'b h d l, b h m l -> b h d m l', v, x_i).sum(dim=3)
#             # v = self.dropout(v * x_i).sum(dim=2)  # Element-wise multiplication followed by summation
#             # torch.einsum('b h a d l, b h m a l -> b h d m l', v, x_i)
#             # torch.einsum('b h d l, b h m l -> b h d m l', v, x_i)
#             v = self.fft_long_conv(outer_prod, h_i, self.B[i], read_positions)
...
#                 original_shape, hyena_out, read_positions, start_indices, batch_indices
#             )
#
#         return hyena_out
